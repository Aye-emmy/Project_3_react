...
const AdminDashboard = () => {
    ...
    const fetchUserData = useCallback(async () => {
        ...
        try {
            setLoading(true);
            setError('');
            const response = await axios.get('https://project-3-node.onrender.com/api/getUserAccount', {
                headers: {
            ...
            if (response.data && response.data.data) {
                ...
                setAccountAmount(userData.user?.accountAmount || '0');
                setAccountName(userData.user?.accountName || '');
                setAccountNumber(userData.user?.accountNumber || '');
    ...
    const navLinks = [
        { path: '/admin-dashboard', label: 'Account', icon: 'fa-solid fa-house' },
        { path: 'transfer', label: 'Transfer', icon: 'fa-solid fa-paper-plane' },
        { path: 'receipt', label: 'Receipts', icon: 'fa-solid fa-receipt' },
        { path: 'allUser', label: 'All Users', icon: 'fa-solid fa-users-gear' },
        { path: 'allReceipt', label: 'All Receipts', icon: 'fa-solid fa-file-invoice-dollar' },
        { path: 'userDetails', label: 'User Details', icon: 'fa-solid fa-user' }
    ];
    const isLandingPage = location.pathname === '/admin-dashboard' || location.pathname === '/admin-dashboard/';
                ...
                <ul className="nav-list list-unstyled flex-column mb-auto">
                    {navLinks.map((link) => {
                        const isActive = location.pathname === link.path ||
                            (link.path === '/admin-dashboard' && isLandingPage) ||
                            location.pathname === `/admin-dashboard/${link.path}`;
